...
public class UsersSecurityConfigrations extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable();
		 ...
		 //@@ THIS LINE OF CODE PERMITS ALL INCOMING REQUEST
		// http.authorizeRequests().antMatchers("/users/**").permitAll();

		//@@ but this one permit request with / having certian ip address
		http.authorizeRequests().antMatchers("/**").hasIpAddress(environment.getProperty("gateway.ip"))
		        .and()
			    .addFilter(getAuthenticationFilter()); //@@ INVOKING CUSTOM MADE AUTHENTICATION-FILTER from FILTER.CLASS
        
		//@@ TO DISABLE H2-CONSOLE  
